---
title: Execvision Charts
summary: Fun with d3 and typescript
tags: ['typescript', 'charts']
draft: false
layout: PostSimple
date: '2019-01-01'
---
...
![9 box happy](/static/images/execvision/9box.png)

The following snippet is the high level `AvatarScatterPlot` `render()` method. I like how clean it turned out:

```tsx
<FlexWrapper>
  <Chart bordered={false} margins={margins} svgDimensions={svgDimensions}>
    ...
    {reps.map(({ rep, coordinate, additionalInfo }, i) => (
      <FancyAvatarPoint
        ...
        highlight={shouldHighlight(coordinate)}
        rep={rep}
        additionalInfo={additionalInfo}
        defaultAvatar={defaultAvatar}
      />
    ))}
    {repsWithoutPlans.length !== 0 && arrowPointer && arrowDelta && (
      <CalloutArrow
        from={arrowPointer}
  ...
  <AdditionalInfo>
    <CalloutCard title="Who to coach?" extra={this.getExtraTooltipInfo(repsWithoutPlans)} />
...
![heatmap 1](/static/images/execvision/heatmap1.png)
![heatmap 2](/static/images/execvision/heatmap2.png)
 ...
 */
export const withBucketing = <P extends WithBucketingProps, U extends WithBuckets>(
...
![sparkline 1](/static/images/execvision/sparkline-singular.png)

If we showed multiple for you to compare, we'd hide the underlying data until you hovered/pinned on the trend.

![sparkline 2](/static/images/execvision/sparkline-multiple.png)

No sparkline is complete without fancy animations -

<video controls>
  <source src="/static/videos/execvision/sparkline-draw.mp4" />
</video>
<br />
...
<video controls>
  <source src="/static/videos/execvision/progress-circle.mp4" />
</video>
